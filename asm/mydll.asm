...
vLoop:
    ...
    call    strcmp ; strcmp(v->vertexName, sourceVertex)
    test    eax, eax ; set JE if ZR is set
    jne     vLoopNoMatch ; no match found
    mov     rax, QWORD PTR [rbp-64] ; v
    mov     DWORD PTR [rax+16], 0 ; v->sumOfWeights = 0
...
v2LoopNoMatch:
    ...
    je      v2LoopMatch ; move to the next pointer
    mov     rax, QWORD PTR [rbp-80] ; v2
    mov     eax, DWORD PTR [rax+16] ; v2->sumOfWeights
    cmp     DWORD PTR [rbp-36], eax ; minimumDistance = v2->sumOfWeights
    jle     v2LoopMatch ; move to the next pointer
    mov     rax, QWORD PTR [rbp-80] ; v2
    mov     eax, DWORD PTR [rax+16] ; v2->sumOfWeights
