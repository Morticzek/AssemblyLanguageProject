...
void printVertices(struct Connection* headConnection, struct Vertex* headVertex, double execTime)
{
    ...
    //    FILE* fileWriteStream = fopen("output.txt", "w+");
    //    fputs("Vertex - Previous - Distance", fileWriteStream);
...
void uniqueList(struct Connection* headConnection, struct Vertex* headVertex, char* sourceVertex)
{
    ...
    printVertices(headConnection, headVertex, exec_time);
}
...
void loadData(char* fileName, char* sourceVertex)
{
    ...
    extractData(headConnection, sourceVertex);
}
